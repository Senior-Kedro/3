...
int check(int n) {
    ...
    while (n > 0) {
        ...
        // If the current digit is 5, increment the consecutiveFives count
        if (number == 5) {
            consecutiveFives++;
        ...
        // Reset the consecutiveFives count if the current digit is not 5
        else {
            consecutiveFives = 0;
        }
    }
...
int countNumbers(int r) {
    // Base cases
    if (r == 1)
        return 2;  // Only two possibilities: 5 and 9
    if (r == 2)
        return 4;  // Four possibilities: 55, 59, 95, 99
